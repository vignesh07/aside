...
const PRICES: ModelPrice[] = [
  ...
  price('anthropic', /^claude-(?:fable|mythos)-5(?:-|$)/, 10, 1, 12.5, 50),
  price('anthropic', /^claude-opus-(?:5|4-[5-8])(?:-|$)/, 5, 0.5, 6.25, 25),
  price('anthropic', /^claude-opus-4(?:-1)?(?:-|$)/, 15, 1.5, 18.75, 75),
  // Sonnet 5 introductory pricing through 2026-08-31.
  price('anthropic', /^claude-sonnet-5(?:-|$)/, 2, 0.2, 2.5, 10),
  price('anthropic', /^claude-sonnet-4(?:-[5-6])?(?:-|$)/, 3, 0.3, 3.75, 15),
  price('anthropic', /^claude-haiku-4-5(?:-|$)/, 1, 0.1, 1.25, 5),
  price('anthropic', /^claude-(?:3-5-haiku|haiku-3-5)(?:-|$)/, 0.8, 0.08, 1, 4),
...
function estimateWithPrice(
  ...
  return (
    ...
    counts.cacheWriteInputTokens * modelPrice.cacheWrite +
...
function price(
  ...
  return { provider, matches, input, cachedInput, cacheWrite, output };
}
