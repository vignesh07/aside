...
function countsFromRow(row: UsageAggregateRow): UsageTokenCounts {
  return {
    ...
    cacheWrite5mInputTokens: safeCount(row.cache_write_5m_input_tokens),
    cacheWrite1hInputTokens: safeCount(row.cache_write_1h_input_tokens),
...
function rowTotal(row: UsageAggregateRow): number {
  return safeCount(row.input_tokens) +
    ...
    safeCount(row.cache_write_5m_input_tokens) +
    safeCount(row.cache_write_1h_input_tokens) +
